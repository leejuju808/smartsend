...
// POST /api/inbox/replies/[threadId]/unsnooze - Un-snooze a thread
...
export async function POST(
  ...
  if (updateError) {
    console.error("Error unsnoozing thread:", updateError);
  ...
  if (lead) {
    ...
    await logContactActivity({
      ...
      contactId: lead.id,
      type: 'status_changed',
      title: 'Thread unsnoozed',
      description: 'Thread manually unsnoozed',
      userId: user.id,
      ...
      meta: {
        ...
        reason: 'manual',
        thread_id: threadId,
        previous_snoozed_until: thread.snoozed_until,
      },
    });
  }
