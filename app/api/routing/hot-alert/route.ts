// app/api/routing/hot-alert/route.ts
// Block 9600 — Smart Routing v1: Hot Lead Alert Endpoint

import { NextRequest, NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { sendHotLeadAlert } from "@/lib/routing/emailNotifications";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Send email alert
    const emailResult = await sendHotLeadAlert({
      accountId,
      contact: {
    ...
    console.error("Hot alert endpoint error:", error);
    return NextResponse.json(
