...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (userError) {
    console.error("archive thread auth lookup failed", userError);
  ...
  if (updateError) {
    console.error("archive thread update failed", updateError);
  ...
  if (activityError) {
    console.warn("archive thread activity insert failed", activityError);
  }
