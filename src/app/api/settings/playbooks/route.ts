...
// POST /api/settings/playbooks - Update playbooks settings
export async function POST(req: Request) {
  try {
    ...
    const { playbooks } = body;

    if (!playbooks) {
      return NextResponse.json({ error: "playbooks settings required" }, { status: 400 });
    }
    ...
    await logTeamActivity({
      ...
      title: "Playbooks settings updated",
      metadata: { section: "playbooks" }
    });
