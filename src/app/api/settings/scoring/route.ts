...
// POST /api/settings/scoring - Update lead scoring settings
export async function POST(req: Request) {
  try {
    ...
    if (!scoring) {
      return NextResponse.json({ error: "scoring settings required" }, { status: 400 });
    }
    ...
    await logTeamActivity({
      ...
      title: "Lead scoring rules updated",
      metadata: { section: "scoring" }
    });
