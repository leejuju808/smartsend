...
// POST /api/settings/sending - Update sending settings
export async function POST(req: Request) {
  try {
    ...
    if (!sending) {
      return NextResponse.json({ error: "sending settings required" }, { status: 400 });
    }
    ...
    await logTeamActivity({
      ...
      title: "Sending settings updated",
      metadata: { section: "sending" }
    });
