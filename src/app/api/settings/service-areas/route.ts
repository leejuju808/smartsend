...
export async function POST(req: NextRequest) {
  try {
    ...
    const { serviceAreas: updateData } = body;

    if (!updateData) {
      return NextResponse.json({ error: "serviceAreas data is required" }, { status: 400 });
    }
    ...
    if (result.error) {
      console.error(`Error updating service areas:`, result.error);
