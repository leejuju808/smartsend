...
import { normalizeMailgun } from "@/lib/replies/normalize";
...
export async function POST(req: Request) {
  try {
    verifySecret(req, process.env.WEBHOOK_SECRET_MAILGUN);
    const ownerUserId = requireUserIdFromQuery(req);
    const bodyClone = req.clone();

    const normalized = await normalizeMailgun(req);
    ...
    let raw: any = {};
    const contentType = bodyClone.headers.get("content-type") || "";
    if (contentType.includes("multipart/form-data")) {
      const form = await bodyClone.formData();
      raw = Object.fromEntries(form.entries());
    } else {
      raw = await bodyClone.json().catch(() => ({}));
    }
