...
export async function POST(request: NextRequest) {
  try {
    ...
    // Stop warmup by setting is_active to false
    ...
    if (updateError) {
      console.error("Error stopping warmup:", updateError);
      return NextResponse.json({ error: "Failed to stop warmup" }, { status: 500 });
    }

    return NextResponse.json({
      success: true,
      message: "Mailbox warmup stopped successfully"
    });

  } catch (error) {
    console.error("Error in warmup stop:", error);
