-- 1) SQL — RLS for meeting tables (idempotent)

alter table if exists public.meeting_prefs    enable row level security;
alter table if exists public.meeting_intents  enable row level security;
alter table if exists public.meeting_slots    enable row level security;
...
-- 2) SQL — Database → Edge Function HTTP trigger (idempotent)
...
-- 4) Small DX & Perf touches (indexes)
