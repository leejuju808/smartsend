-- OOO auto-nudge support (idempotent)
...
-- C) Helper: compute next business day at a target hour:min (tz-naive here; good enough on server UTC)
create or replace function public.next_business_morning(
  p_date date,
  p_hour int default 8,
  p_min int default 30
) returns timestamptz
language plpgsql
immutable
as $$
...
begin
  ...
  if v_thread.ai_return_date is not null then
    ...
    v_due := public.add_business_days(now(), 3)::date::timestamptz + time '08:30';
  end if;
  ...
  if v_thread.ooo_autonudge_variant_id is not null then
    ...
    order by random() * coalesce(1.0 / nullif(v.weight, 0), 1.0)
    limit 1;
  end if;
