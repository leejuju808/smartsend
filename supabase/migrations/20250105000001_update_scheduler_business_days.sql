-- Update schedule_followups_for_campaign to use next_business_window_tz
...
begin
  ...
  plan as (
    select
      e.lead_id,
      public.next_business_window_tz(
        e.last_sent + (v_offset || ' days')::interval,
        v_start, v_end,
        public.pick_lead_timezone(e.lead_id, p_campaign),
        public.pick_lead_country(e.lead_id, p_campaign),
        (select skip_weekends from public.campaigns where id = p_campaign),
        (select skip_holidays from public.campaigns where id = p_campaign)
      ) as scheduled_at
    from eligible e
