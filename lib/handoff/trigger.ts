...
// Checks if handoff should be triggered and initiates it
...
export async function checkAndTriggerHandoff(threadId: string): Promise<void> {
  const supabase = createClient(supabaseUrl, supabaseServiceKey);

  try {
    // Check if handoff should be triggered
    const { data: shouldTrigger, error: checkError } = await supabase.rpc(
    ...
    if (!campaign || campaign.handoff_mode !== 'auto') {
      return; // Only auto-trigger if mode is 'auto'
    }
    ...
    console.error('Error in checkAndTriggerHandoff:', error);
